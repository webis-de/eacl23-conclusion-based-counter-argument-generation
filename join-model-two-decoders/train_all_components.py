...
def main(args):
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch in  tqdm.tqdm(train_loader):
            # print(batch['conclusions'][0])
            # print(batch['input_ids'][0])
            # print(batch['claim_target_encodings'][0])
            # print(batch['argument_target_encodings'][0])
            ...
            generated_conclusion = model.conclusion_model.generate(input_ids, min_length=args.max_claim_target, max_length=args.max_claim_target)
            ...
            jointTxt_stance = [a + " </s> " + b  for a,b, in zip(batch_conclusions, generated_conclusion)] # for ms
            ...
            #5. Running through the counter-claim decoder
            counter_claim_outputs = model.counter_claim_model(input_ids, attention_mask=attention_mask, labels = claim_targets, return_dict=True)

            counter_claim_lhs    = counter_claim_outputs.encoder_last_hidden_state # this is actually decoder's last hidden state, look at model.py for details
            eos_mask = claim_targets.eq(model.counter_claim_model.config.eos_token_id)
            counter_claim_sentence_embedding = counter_claim_lhs[eos_mask, :].view(counter_claim_lhs.size(0), -1, counter_claim_lhs.size(-1))[:, -1, :]

            #6. Compute the stance between the generated counter-claim and the input conclusion
            outputs_reference = model.counter_claim_model(**batch_conclusion_encoding, return_dict=True)
            reference_last_hidden_state = outputs_reference.encoder_last_hidden_state #decoder's last hidden state
            refernce_sentence_embedding = reference_last_hidden_state[:,-1,:]
            concat_embedding_stance = torch.cat((counter_claim_sentence_embedding, refernce_sentence_embedding), 1)
            approx_logits_classification = model.counter_claim_model.classification_head(concat_embedding_stance)
            approx_logits_classification = torch.sigmoid(approx_logits_classification)

            #7. Compute the real stance between the generated counter-claim and the input conclusion through the teacher model
            generated_counter_claim = model.counter_claim_model.generate(input_ids, min_length=args.max_claim_target, max_length=args.max_claim_target)
            generated_counter_claim = tokenizer.batch_decode(generated_counter_claim, skip_special_tokens=True, clean_up_tokenization_spaces=False)
            jointTxt_stance = [a + " </s> " + b  for a,b, in zip(batch_conclusions, generated_counter_claim)] # for ms
            jointTxt_stance = stance_classifier_teacher_tokenizer.batch_encode_plus(jointTxt_stance, return_tensors="pt", max_length=args.max_claim_target * 2, truncation=True, padding=True)
            jointTxt_stance = jointTxt_stance.to(device)
            real_logits_classification = stance_classifier_teacher_model(**jointTxt_stance)[0]
            real_logits_classification = torch.sigmoid(real_logits_classification).to(device)
            
            #Compute the counter-claim stance loss 
            loss_fct = MSELoss()
            counter_claim_stance_loss = loss_fct(approx_logits_classification, real_logits_classification)
            ...
            generated_counter_argument = model.counter_argument_model.generate(input_ids, min_length=args.max_argument_target, max_length=args.max_argument_target)
            ...
            jointTxt_stance = [a + " </s> " + b  for a,b, in zip(batch_conclusions, generated_counter_argument)] # for ms
            ...
            #12. Compute the stance between the generated counter-argument and the generted counter-claim        
            concat_embedding_stance = torch.cat((counter_argument_sentence_embedding, counter_claim_sentence_embedding), 1)
            approx_logits_classification = model.counter_argument_model.classification_head(concat_embedding_stance)
            approx_logits_classification = torch.sigmoid(approx_logits_classification)

            #13. Compute the real stance between the generated counter-argument and the counter-claim through the teacher model
            jointTxt_stance = [a + " </s> " + b  for a,b, in zip(generated_counter_claim, generated_counter_argument)] # for ms
            jointTxt_stance = stance_classifier_teacher_tokenizer.batch_encode_plus(jointTxt_stance, return_tensors="pt", max_length=args.max_argument_target + args.max_claim_target, truncation=True, padding=True)
            jointTxt_stance = jointTxt_stance.to(device)
            real_logits_classification = stance_classifier_teacher_model(**jointTxt_stance)[0]
            real_logits_classification = torch.sigmoid(real_logits_classification).to(device)
            
            #Compute the counter-argument stance loss 
            loss_fct = MSELoss()
            claim_vs_argument_stance_loss = loss_fct(approx_logits_classification, real_logits_classification)

            avg_loss_claim_gen.append(float(counter_claim_outputs.loss))
            avg_loss_argument_gen.append(float(counter_argument_outputs.loss))
            avg_loss_conc_gen.append(float(conclusion_outputs.loss))
            avg_loss_claim_to_conclusion_stance.append(float(counter_claim_stance_loss))
            avg_loss_argument_to_conclusion_stance.append(float(counter_argument_stance_loss))
            avg_loss_argument_to_claim_stance.append(float(claim_vs_argument_stance_loss))
            avg_loss_conclusion_stance.append(float(conclusion_stance_loss))

            loss = counter_claim_outputs.loss + conclusion_outputs.loss + counter_argument_outputs.loss + alpha1 * counter_claim_stance_loss + alpha2 * counter_argument_stance_loss + alpha3 * claim_vs_argument_stance_loss
            ...
            if num_global_steps % args.eval_steps == 0:
                ...
                eval_loss_claim_gen, eval_loss_argument_gen, eval_loss_conc_gen, eval_loss_claim_to_conclusion_stance, eval_loss_argument_to_conclusion_stance, eval_loss_argument_to_claim_stance = run_evaluation(args, dev_loader)
                print('Validation losses: {}, {}, {}, {}, {}, {},'.format(eval_loss_claim_gen, eval_loss_argument_gen, avg_loss_conc_gen, eval_loss_claim_to_conclusion_stance, eval_loss_argument_to_conclusion_stance, eval_loss_argument_to_claim_stance))
                ...
                with open(results_path_prefix +'/training_log.txt', 'a') as file:
                    ...
                    file.writelines(['Source: ', tokenizer.decode(batch['input_ids'][0], skip_special_tokens=True, clean_up_tokenization_spaces=False), '\n\n', 
                                     'Claim Target: ', generated_counter_claim[0], '\n\n', 
                                     'Argument Target: ', generated_counter_argument[0], '\n\n',
                                     'Validation Losses: {}, {}, {}, {}, {},'.format(eval_loss_claim_gen, eval_loss_argument_gen, eval_loss_claim_to_conclusion_stance, eval_loss_argument_to_conclusion_stance, eval_loss_argument_to_claim_stance), '\n\n\n'])
...
def run_evaluation(args, dev_loader):
    ...
    return  np.mean(avg_loss_claim_gen), np.mean(avg_loss_argument_gen), np.mean(avg_loss_conc_gen), np.mean(avg_loss_claim_to_conclusion_stance), np.mean(avg_loss_argument_to_conclusion_stance), np.mean(avg_loss_argument_to_claim_stance)
